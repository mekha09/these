...
\begin{equation*}
J = xm\biggl(\frac{dy}{dt}\biggr) - ym\biggl(\frac{dx}{dt}\biggr) = xp_y - yp_x.
\end{equation*}
...
\begin{equation*}
\bm{\tau_{tot}} = \bm{\tau_{ext}} = \frac{d\bm{J}}{dt}. 
\end{equation*}
...
Une façon d'écrire cette propriété de ''garder la physique inchangée'' est la suivante : si j'applique $\hat{Q}$ au système, alors $\ket{\psi_1}$ est transformé en $\ket{\psi'_1}=\hat{Q}\ket{\psi_1}$, et $\ket{\psi'_2}=\hat{Q}\ket{\psi_2}$. Mais la physique est inchangée par $\hat{Q}$, si on attend un temps $t$ on obtient :
\begin{align*}
...
Supposons maintenant qu'une fois l'opérateur $\hat{Q}$ appliqué à notre état, on obtienne un état qui soit \textit{physiquement le même}. C'est-à-dire que $\hat{Q}\ket{\psi_0}$ est égal à $\ket{\psi_0}$ à un facteur de phase près qu'on note $e^{i\delta}$. Après un temps $t$, on obtient $\ket{\psi_2}$ à qui on peut appliquer $\hat{Q}$ :
\begin{align*}
...
\end{equation*}
On peut montrer \mycite{cohen} que $\delta$ est nécessairement proportionnel à $\theta$. Ainsi,
\begin{equation*}
