...
%Pour traiter le champ électromagnétique comme un système quantique, il est nécessaire de passer d'une description continue à un  ensemble discret de variables. Par exemple, on peut partir d'un potentiel vecteur $\bm{A}(\bm{r},t)$ respectant la condition de transversalité $\mathrm{div}\bm{A}=0$ et le décomposer comme une somme d'ondes planes de vecteur d'onde $\bm{k}_i$, où $i$ parcourt un espace discret infini. \`A partir de cette décomposition, on peut calculer l'hamiltonien du système qui se présente sous la forme d'une somme d'oscillateurs harmoniques 1D. Il reste ensuite à passer à des variables canoniques pour quantifier le système (voir par exemple le chapitre I de \mycite{Landau1982a}). Dans cette description, l'énergie et la quantité de mouvement du champ sont donnés par :
%\begin{align*}
%E~&=~\sum_{\bm{k},\alpha}{N_{\bm{k},\alpha} \hbar\omega}, \\
%P~&=~\sum_{\bm{k},\alpha}{N_{\bm{k},\alpha} \hbar\bm{k}}
%\end{align*}
%
%Pour expliquer ces expressions, on introduit le concept de photon, ou quantum de radiation électromagnétique : le champ est vu comme un ensemble de particules ayant chacune une énergie $\hbar\omega$ et une quantité de mouvement $\hbar\bm{k}$. $N_{\bm{k},\alpha}$, appelé \textit{nombre d'occupation}, est alors vu comme le nombre de photons ayant la quantité de mouvement $\bm{k}$ et la polarisation $e^{(\alpha)}$. Nous verra plus tard la définition de la polarisation du photon, et en particulier qu'elle est analogue à son spin. \par
%On peut également écrire la fonction d'onde du photon, mais il faut effectuer une distinction importante avec le cas d'une particule telle qu'un atome. En effet, le photon se déplace à la vitesse de la lumière, ce qui oblige à utiliser une théorie relativiste de la mécanique quantique. Dans ce cas, la fonction d'onde ne peut pas être vue comme l'amplitude de probabilité de trouver le photon à un endroit donné. En effet, on vient de voir que l'énergie du photon s'écrit $E = \hbar\omega = \hbar\left|\bm{k}\right|\rmc=\left|\bm{p}\right|\rmc$. On peut alors écrire l'incertitude sur la position du photon comme :
%\begin{equation*}
